...
The project is installable as a Progressive Web App (PWA). Deploy it over HTTPS, open it in Chrome on Android, then choose **Install app** from Chrome's menu. It launches as a full-screen portrait app and works offline after the first successful load.

The portrait main screen contains only the QSO log, timer/start control, receiving entry, and F1–F8 message keys. Tap **Settings** to access station, band condition, contest, recording, transcript, statistics, score, language, and theme options.
